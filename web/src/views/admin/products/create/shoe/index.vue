...
const formState = reactive<ShoeCreateFormState>({
    name: "",
    price: 0,
    discount_percent: 0,
    in_stock: 0,
...
const onOk = async () => {
    ...
    try {
        ...
        await api.post(`/products/shoes`, data)
        router.push({ name: "Product Management - Create" })
        Modal.success({
            title: "Success",
...
<template>
    <a-modal :visible="$route.name === 'Product Management - Create Shoe'" title="Thêm giày" ok-text="Thêm" cancel-text="Hủy" @ok="onOk" width="800px" @cancel="onCancel">
        <a-form ref="formRef" :model="formState" layout="horizontal" :label-col="{ span: 6 }" label-align="right" name="form_in_modal" class="relative">
