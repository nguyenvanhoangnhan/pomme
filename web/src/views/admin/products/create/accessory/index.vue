...
interface AccessoryCreateFormState {
    name: string
    price: number
    in_stock: number
...
const formState = reactive<AccessoryCreateFormState>({
    name: "",
    price: 0,
    discount_percent: 0,
    in_stock: 0,
...
const onOk = async () => {
    ...
    try {
        ...
        await api.post(`/products/accessory`, data)
...
<template>
    <a-modal :visible="$route.name === 'Product Management - Create Accessory'" title="Thêm phụ kiện" ok-text="Thêm" cancel-text="Hủy" @ok="onOk" width="800px" @cancel="onCancel">
