...
interface ClothesCreateFormState {
    name: string
    price: number
    in_stock: number
...
const formState = reactive<ClothesCreateFormState>({
    name: "",
    price: 0,
    discount_percent: 0,
    in_stock: 0,
...
const onOk = async () => {
    ...
    try {
        ...
        await api.post(`/products/clothes`, data)
...
const CATEGORIES = ["Tee", "Hoodie", "Sweatshirt"]
</script>

<template>
    <a-modal :visible="$route.name === 'Product Management - Create Clothes'" title="Thêm áo quần" ok-text="Thêm" cancel-text="Hủy" @ok="onOk" width="800px" @cancel="onCancel">
