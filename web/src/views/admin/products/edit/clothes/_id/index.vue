...
interface ClothesEditFormState {
    id: number
    product_id: number
    name: string
    price: number
...
const formState = reactive<ClothesEditFormState>({
    id: 0,
    product_id: 0,
    name: "",
    price: 0,
...
const onOk = async () => {
    ...
    try {
        ...
        await api.put(`/products/clothes`, data).then(() => {
...
onMounted(async () => {
    ...
    const res = await api.get(`/products/clothes/${id}`)
...
<template>
    <a-modal :visible="$route.name === 'Product Management - Edit Clothes'" title="Chỉnh sửa sản phẩm" ok-text="Sửa" cancel-text="Hủy" @ok="onOk" width="800px" @cancel="onCancel">
