...
const formState = reactive<ShoeEditFormState>({
    id: 0,
    product_id: 0,
    name: "",
    price: 0,
...
const onOk = async () => {
    ...
    try {
        ...
        await api.put(`/products/shoes`, data).then(() => {
...
onMounted(async () => {
    ...
    const res = await api.get(`/products/shoes/${id}`)
    const shoe = res.data as ShoeWithProduct
    ...
    formState.series = shoe.series
    formState.gender = shoe.gender
    formState.shape = shoe.shape
})
...
const SERIES = ["GSmith", "Crispin", "Shizuka", "Rhode"]
const GENDERS = ["Nam", "Nữ", "Unisex"]
const SHAPES = ["Low Top", "High Top"]
</script>

<template>
    <a-modal :visible="$route.name === 'Product Management - Edit Shoe'" title="Chỉnh sửa sản phẩm" ok-text="Sửa" cancel-text="Hủy" @ok="onOk" width="800px" @cancel="onCancel">
        ...
        <a-form v-else ref="formRef" :model="formState" layout="horizontal" :label-col="{ span: 6 }" label-align="right" name="form_in_modal" class="relative">
            ...
            <div style="max-height: 480px; overflow-y: scroll; padding-inline: 1rem">
                ...
                <a-form-item name="series" label="Dòng giày" :rules="[{ required: true, message: 'Bạn chưa chọn trường này!' }]">
                    <a-select v-model:value="formState.series">
                        <a-select-option v-for="serie in SERIES" :key="serie" :value="serie">{{ serie }}</a-select-option>
                    </a-select>
                </a-form-item>
                <a-form-item name="series" label="Dáng">
                    <a-select v-model:value="formState.shape">
                        <a-select-option v-for="(shape, index) in SHAPES" :key="shape" :value="index">{{ shape }}</a-select-option>
                    </a-select>
                </a-form-item>
                <a-form-item name="gender" label="Giới tính">
                    <a-radio-group v-model:value="formState.gender">
                        <a-radio v-for="(gender, index) in GENDERS" :value="index" :key="index">
                            {{ gender }}
                        </a-radio>
                    </a-radio-group>
                </a-form-item>
